        ...
        {stages.map((stage, index) => (
          <div key={index} className="relative flex pb-6 last:pb-0">
            ...
            {index !== stages.length - 1 && (
              <div
                className="absolute top-12 left-6 -ml-px h-full w-[1px] bg-blue-950/50"
                aria-hidden="true"
              />
            )}

            {/* Circle with number */}
            <div className="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-full border-1 border-blue-950 bg-gray-100 text-blue-950">
              {stage.number}
            </div>

            <div className="mt-6 -mr-3 h-[1px] w-9 bg-blue-950/50" />

            {/* Content */}
            <div className="pt-3">
              <h3 className="mb-4 pl-7 text-base font-bold text-blue-950">{stage.title}</h3>
              <ul className="mb-3 space-y-2 text-sm">
                {stage.items.map((item, itemIndex) => (
                  <li key={itemIndex} className="flex items-center gap-4 text-blue-950/80">
                    <span className="ml-1 h-1.5 w-1.5 shrink-0 rounded-full bg-blue-950" />
