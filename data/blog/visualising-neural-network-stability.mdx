...
socialImage: /static/images/posts/neural-networks/visulising-stability.png
---

<KeyTakeaways
  items={[
    'Tiny decisions in architecture or training data can have huge effects on stability of a model.',
     'A 2D loss-landscape curve of the whole neural network, which helps us visualise its stability.',
     'Only a human decision-maker can decide how to evaluate the model\'s current stability.'
  ]}
/>
...
Today it rained 2 inches, and we sold 6 gallons, so our assumption looks correct so far. Based on this our weight and bias could be anywhere on this line.
...
**Different combinations of weights and biases could explain this exact same outcome. For example, maybe our bias is actually 5, and we sell an extra half a gallon for every inch of rainfall.** For now, we can just see how much rain there is and how much coffee we sell over the next few days.
...
- The majority of the ink in the contour plot is dedicated to information with limited benefit.
...
Our model is doing well up to this point, so we decide to just wait until a rainy weekend before we continue training. The lowest-tech solution is normally the best after all.
...
## A Tool To Visulise Model Stability
...
* Norms / condition number: [Andy Jones — Condition numbers](https://andrewcharlesjones.github.io/journal/condition-numbers.html)
* Derivative: [3B1B — The paradox of the derivative](https://www.youtube.com/watch?v=9vKqVkMQHKk)
* Gradient descent: [3B1B — Gradient descent](https://www.youtube.com/watch?v=IHZwWFHWa-w)
* Bridge (eigenvalues → stability): [Setosa — Eigenvectors and eigenvalues](https://setosa.io/ev/eigenvectors-and-eigenvalues/)